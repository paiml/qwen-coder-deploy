...
**Roadmap:** Per-M CUDA graph (multi-token dispatch) → fused Q4K GEMM → continuous batching → paged KV → Phase 2 (cache intelligence). **⚠️ PMAT-283 timing FALSIFIES PMAT-280:** scheduler-level pipelining has 0% ROI — 99.99% of step time is inside `batched_decode_step()` (lock=0µs, sched=0µs, dist=1µs). Event-based sync enables pipelining of H2D + embedding overlap with GPU (small gain), but the binding bottleneck is per-token kernel dispatch. **Per-M CUDA graph (vLLM-style multi-token CUTLASS dispatch) is the highest-ROI change** — PMAT-282 proved multi-M graphs provide +18-27%. **Gap decomposition (PMAT-179→277):** gap = decode_rate × scheduling_utilization, validated within 1%. With B32 iter sched: **sched_util=0.94-0.98× at c≤32** (near-optimal). Remaining gap is purely decode_rate (0.45-0.52×). At c≥64: queueing penalty (0.24-0.48×) from BATCH=32 cap offsets decode advantage (0.98-1.98×). **Phase projections (PMAT-265→267→279):** event sync + per-M graph → **0.66-0.79× vLLM** (50-80% CPU-GPU overlap). Per-step at c=4: GPU 7.4ms + serving 6.3ms. c=1 decomposition (PMAT-279): GPU 6.29ms + serving 0.54ms + graph savings 0.83ms. Serving overhead grows with c (0.54ms c=1 → 6.3ms c=4 → ~8.4ms c=16). With kernel fusion: 0.85-1.00× vLLM. **Iso-quality gap (PMAT-263):** Score≥70: 2.1× (was 5.3× at B16 B&S). **Quality crossover (PMAT-192/261):** realizr BEATS vLLM at c≈64-128 (decode advantage 0.98-1.98× with B32). Target: ≥0.90× vLLM at c=8 after Phase 1 (requires kernel fusion + pipelining).
...
2. **Batch scheduler: BLOCKER.** Legacy `cuda_batch_scheduler.rs` (430 LOC) implements batch-and-step with synchronous `process_cuda_batch()` — waits for entire batch to complete before starting next. `iteration_scheduler.rs` (459 LOC) exists as framework for per-iteration dispatch (opt-in via `ITERATION_SCHEDULER=1`) but is **incomplete** — no mid-batch slot addition, no prefill chunking. `gguf/batch_scheduler.rs` has fixed-size slots array. **Refactor: ~500-700 LOC.** Replace blocking call with channel-based iteration loop, implement `add_slot_to_batch()`.
...
| 5.23.0 | 2026-03-19 | **PMAT-283: Event sync implementation target.** Source analysis of realizr decode path. Critical sync bottleneck: `reduces.rs:92` `stream.synchronize()` blocks CPU 6.3ms at c=4. Zero event-based sync in codebase. Implementation plan: 4 files, ~100 LOC. `streams.rs` (event wrappers) + `reduces.rs` (sync→event) + `iteration_scheduler.rs` (overlap token distribution with GPU). Projected: c=4 285→520 tok/s (0.89× vLLM). Falsification: <450 = non-overlappable component. |
