...
| **PMAT-245** | **Serial c=32 same-session — realizr+vLLM stable, llama.cpp HEAD regression −55%** | **realizr 868.6 (+0.2%), vLLM 2900.6 (+5.2%), llama.cpp 426.7 (−54.7%)** | ✅ MEASURED. Serial isolated deployment on yoga RTX 4060L. realizr matches PMAT-177 exactly (868.6 vs 867.3, +0.2%). vLLM +5.2% (within ±10% high-c variance). **llama.cpp regression: 943.2→426.7 (−54.7%)** despite identical avg_tok (90.7 vs 90.6) and latency (3195 vs 3297ms). throughput_rps halved: 10.4→4.7. Cause: forjar builds from llama.cpp HEAD each deploy — likely a regression in the Mar 15→18 window. Does NOT affect prior conclusions since all serial c=1/4/8/16 measurements were done in the current session (consistent build). Per-request decode: realizr 57.1 vs llama.cpp 58.0 — **near-parity at c=32** (was 1.28× advantage at c=16, 1.45× at c=8). |
...
|---------|------|---------|
| 3.98.0 | 2026-03-18 | **PMAT-245: Serial c=32 same-session.** realizr 868.6 (+0.2% vs PMAT-177), vLLM 2900.6 (+5.2%). **llama.cpp HEAD regression: 943.2→426.7 (−54.7%)**. throughput_rps halved despite identical latency and avg_tok — forjar builds from HEAD each deploy. Per-request decode: realizr 57.1 vs llama.cpp 58.0 = near-parity at c=32 (was 1.28× at c=16, 1.45× at c=8). Extends serial curve to c=32. |
