...
**⚠️ PMAT-221: Critical realizr quality bug at c≥9 with long prompts.** At c=9+ with long prompts (~353 tokens), realizr generates 0 tokens for ALL responses. Threshold is exactly c=8→c=9. Once triggered, the server stays in degraded state until restart. PMAT-220 c=16 data (avg_tok=63) was from partially degraded state; c=16 long-prompt data is INVALID. Root cause likely KV workspace or CUDA graph corruption at M=9 with large prompt KV occupancy. This is a correctness bug, not a performance issue. Affects: any workload with ≥9 concurrent long-prompt requests. vLLM and llama.cpp are unaffected at equivalent concurrency.
...
| **PMAT-221** | **realizr long-prompt quality bug investigation** | **⚠️ BUG: 0-token output at c≥9 with long prompts** | ⚠️ BUG FOUND. realizr generates 0 tokens for ALL responses at c≥9 with long prompts (~353 tokens). Threshold is exactly c=8→c=9. Once triggered, server stays degraded until restart. PMAT-220 c=16 data (avg_tok=63) was from partially degraded state — c=16 long-prompt data is INVALID. Tested: c=1 (OK, 64/64), c=4 (OK), c=8 (OK, 64/64), c=9 (0/0/0/0), c=10-16 (0/0/0/0). Reproducible across fixed:64 and uniform:16,256. vLLM and llama.cpp unaffected at equivalent concurrency. Root cause likely KV workspace or CUDA graph corruption at M≥9 with large prompt KV occupancy. |
...
|---------|------|---------|
| 3.73.0 | 2026-03-17 | **PMAT-221: Critical realizr quality bug — 0-token output at c≥9 with long prompts.** Investigation triggered by PMAT-220 c=16 output degradation (p50=10 tokens). Systematic testing reveals exact threshold: c=8 works perfectly (64/64/64/64 tokens), c=9 produces 0/0/0/0 tokens. Once triggered, server stays degraded until restart. PMAT-220 c=16 data invalidated. Competitive matrix long+hetero row updated to mark c=16 as BUG. Likely root cause: KV workspace or CUDA graph corruption at M≥9 with large prompt KV occupancy (~353 tokens/request × 9 = ~3177 total KV positions). |
