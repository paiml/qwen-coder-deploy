...
|---------|------|---------|
| 2.56.0 | 2026-03-12 | **PMAT-099: Staggered prefill FALSIFIED for short prompts — +10% TTFT, −1.5% aggregate.** Hypothesis: prefilling only the first prompt in Phase 1 and joining remaining slots mid-batch (one-per-decode-step) reduces head-of-line blocking and improves TTFT at c>1. Implementation: `cuda_batch_scheduler.rs` staggered path — Phase 1 calls `batched_setup_and_prefill` with 1 prompt, Phase 2 decode loop joins pending slots via `add_slot_to_batch` (clear_decode_graph → init_prefill_workspace → run_prefill → scatter → init_batched_workspace). **Benchmarks (yoga 4060L, 1900MHz, 60s, isolated, short prompt ~125 tokens):** Staggered c=4: TTFT 84.3ms, decode 62.9 tok/s, aggregate 239.5, 57 requests/0 errors. Non-staggered c=4 (STAGGERED_PREFILL=0): TTFT 76.6ms, decode 63.8 tok/s, aggregate 243.2, 61 requests/0 errors. c=1 baseline: TTFT 19.9ms, decode 148.1 tok/s. **Root cause:** For short prompts, per-slot `add_slot_to_batch` overhead (~14ms each: workspace reinit + prefill + graph clear) exceeds batched multi-prompt prefill cost (~20ms for M=4 total). Staggered makes M=4 cost = 20ms (1 prompt) + 3×14ms (3 joins during decode) = 62ms vs non-staggered = 20ms (all 4 prompts). **When staggered wins:** Long prompts (>500 tokens) where batched prefill cost scales linearly with M×prompt_len, making per-slot join cheaper than all-at-once. **Default:** ON (env `STAGGERED_PREFILL=0` to disable). Keep for long-prompt workloads pending further benchmarks. Also discovered: `--batch` CLI flag routes through `OwnedQuantizedModelCachedSync` (CPU inference, broken for chat completions) — `--gpu` flag is the correct CUDA path. NaN corruption previously reported did NOT reproduce in current build (0/92 at c=8, 0/60 at c=4). |
