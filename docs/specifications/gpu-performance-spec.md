...
‡ realizr generates 0 tokens at c≥9 with long prompts (PMAT-221 quality bug). c=16 data INVALID. llama.cpp --parallel 8 maxes out at c=8.

**PMAT-220 key finding: long prompts WIDEN the gap.** realizr/vLLM: 0.94×, 0.33×, 0.28× (c=1,4,8). TTFT gap explodes: realizr 175ms vs vLLM 21ms at c=4 (8.3×), 330ms vs 23ms at c=8 (14.4×). FP8 2-step prefill BW overhead scales with prompt length as predicted.
...
**PMAT-222 root cause analysis (falsification of total-token hypothesis):**
- **Total-token hypothesis FALSIFIED**: short c=128 works (32×23=736 per-batch tokens per wave), but medium c=18 breaks (18×102=1836). Long c=8 works (8×311=2488 per-batch tokens) — higher than medium c=18 but works fine. Total tokens cannot be the discriminator.
- **Verified thresholds**: short ≤32 slots OK (never breaks), medium c=17 OK / c=18 broken, long c=8 OK / c=9 broken.
- **Per-batch products**: medium 17×102=1734 (OK), long 8×311=2488 (OK), short 32×23=736 (OK). medium 18×102=1836 (BROKEN), long 9×311=2799 (BROKEN).
- **The bug is prompt-length-dependent, not total-token-dependent.** Something in the batched prefill kernel has a per-slot limit that decreases with prompt length, but not linearly proportional to total tokens. Likely a shared memory, workspace, or alignment issue in `batch_prefill.rs` where the allocation size depends on `(seq_len, m)` in a non-trivial way.
- Long-prompt corruption (c≥9) is PERSISTENT (KV cache permanently poisoned until restart). Medium-prompt corruption (c≥18) is TRANSIENT (recovers at lower concurrency, KV entries evicted). vLLM and llama.cpp unaffected.
...
| **PMAT-222** | **Falsify total-token hypothesis from PMAT-221** | **FALSIFIED: bug is prompt-length×slots, not total tokens** | ✅ FALSIFIED. Short c=128 works (32×23=736 per-batch tokens per wave) — higher total tokens than medium c=18 threshold. Precise thresholds: short ≤32 slots OK (never breaks), medium c=17 OK / c=18 BROKEN, long c=8 OK / c=9 BROKEN. Per-batch token products: 17×102=1734 (OK), 8×311=2488 (OK), 32×23=736 (OK), 18×102=1836 (BROKEN), 9×311=2799 (BROKEN). Total tokens cannot explain why long 2488 works but medium 1836 doesn't. The bug is prompt-length-dependent: some shared memory, workspace, or alignment constraint in `batch_prefill.rs` has a per-slot limit that decreases with prompt length in a non-linear way. |
| **PMAT-221** | **realizr long-prompt quality bug investigation** | **⚠️ BUG: 0-token output at c≥9 with long prompts** | ⚠️ BUG FOUND. realizr generates 0 tokens for ALL responses at c≥9 with long prompts (~353 tokens). Threshold is exactly c=8→c=9. Once triggered, server stays degraded until restart. PMAT-220 c=16 data (avg_tok=63) was from partially degraded state — c=16 long-prompt data is INVALID. Tested: c=1 (OK, 64/64), c=4 (OK), c=8 (OK, 64/64), c=9 (0/0/0/0), c=10-16 (0/0/0/0). Medium: c=17 OK (avg_tok=126.2), c=18 BROKEN (avg_tok=62.2, [0,0,128,128]), c=24 worse (avg_tok=1.7), c=32 fully broken (0/0/0/0). Reproducible across fixed:64 and uniform:16,256. vLLM and llama.cpp unaffected. Root cause: prompt-length × slot-count interaction in batched prefill (per PMAT-222 falsification). |
