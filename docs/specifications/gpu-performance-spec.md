...
| **T1a: Kernel fusion + per-M graph 🔑** | PMAT-054 fused Q4K GEMM, per-M CUDA graph capture, kernel count 44→~15 | **PMAT-266: binding constraint. 4.6× GPU kernel gap. Targets 0.80×+ vLLM** |
| **T1b: Dynamo Phase 1** | PMAT-052 paged KV, PMAT-053 paged attention, continuous batching | **Scheduling+batch cap fix. Graph+CB → 0.59-0.71× vLLM (PMAT-266). Full (fusion+CB): 0.82-0.93×** |
